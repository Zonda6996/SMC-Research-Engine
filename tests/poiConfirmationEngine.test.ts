...
function makePoi(overrides: Partial<LiquidityPoiCandidate> = {}): LiquidityPoiCandidate {
	return {
		...
		lastContributionAt: 0, stackNotional: 0, stackShare: 1,
...
it('версия заморожена; пустой вход — пустой выход', () => {
	assert.equal(POI_CONFIRMATION_VERSION, 'poi-confirmation-1.8-sweep-dedup')
...
it('§16.18: дедуп входов «один свип = одна сделка» — две зоны, один пересвип-бар: торгуется сильнейший стек', () => {
	const ltf: Candle[] = [...baseline(7, 0, 110), ...fullLongSequence(0)]
	const strong = makePoi({ id: 'poi-strong', stackNotional: 100, knownAt: 0 })
	const weak = makePoi({ id: 'poi-weak', stackNotional: 5, knownAt: 0 })
	const results = detectPoiConfirmation([weak, strong], ltf)
	const w = results.find(r => r.poiId === 'poi-weak')!.attempts[0]!
	const s = results.find(r => r.poiId === 'poi-strong')!.attempts[0]!
	// Обе попытки вошли с одного финального пересвип-бара (ts 19) в одну сторону.
	assert.equal(w.status, 'entered')
	assert.equal(s.status, 'entered')
	// Побеждает сильнейший стек; слабая помечена дублем и не торгуется.
	assert.equal(s.duplicateEntryOf, null)
	assert.equal(w.duplicateEntryOf, 'poi-strong')
	// Трейс дубля сохранён для QA (вход/исход не стёрты).
	assert.equal(w.outcome, 'tp')
})

it('§16.18: разные пересвип-бары — дубля нет (контроль)', () => {
	// Зона A торгует первую последовательность (окно до ts 40), зона B — вторую (окно с ts 100).
	const ltf: Candle[] = [
		...baseline(7, 0, 110), ...fullLongSequence(0),
		...baseline(79, 28, 110), // отход и перевзвод для второй зоны
		...fullLongSequence(100),
	]
	const a = makePoi({ id: 'poi-a', stackNotional: 100, knownAt: 0, endAt: 40 })
	const b = makePoi({ id: 'poi-b', stackNotional: 5, knownAt: 100 })
	const results = detectPoiConfirmation([a, b], ltf)
	const ea = results.find(r => r.poiId === 'poi-a')!.attempts.find(x => x.status === 'entered')!
	const eb = results.find(r => r.poiId === 'poi-b')!.attempts.find(x => x.status === 'entered')!
	assert.ok(ea && eb)
	// Свипы на разных барах (ts 19 и ts 119) — обе сделки самостоятельные.
	assert.equal(ea.duplicateEntryOf, null)
	assert.equal(eb.duplicateEntryOf, null)
})

it('§16.18: пометка «против импульса» — каузальный ход за impulseBars ЗАКРЫТЫХ HTF-баров, маркер не фильтр', () => {
	const ltf: Candle[] = [...baseline(7, 0, 110), ...fullLongSequence(0)]
	// HTF с шагом 8: последний ЗАКРЫТЫЙ бар на момент входа (ts 23) — бар ts 8 (закрывается в 16).
	// Ход = close(ts 8) / close(10 баров назад) − 1 = 95/110 − 1 ≈ −13.6% — лонг ПРОТИВ обвала.
	const htf: Candle[] = [
		...Array.from({ length: 20 }, (_, k) => ({ timestamp: -160 + k * 8, open: 110, high: 111, low: 109, close: 110, volume: 10 })),
		{ timestamp: 0, open: 110, high: 111, low: 93, close: 96, volume: 30 },
		{ timestamp: 8, open: 96, high: 101, low: 93, close: 95, volume: 10 },
		{ timestamp: 16, open: 95, high: 101, low: 94, close: 100.5, volume: 10 },
	]
	const [result] = detectPoiConfirmation([makePoi()], ltf, htf)
	const attempt = result!.attempts[0]!
	assert.equal(attempt.status, 'entered') // маркер ничего не фильтрует
	assert.ok(attempt.impulseRet != null && attempt.impulseRet < -0.1)
	assert.equal(attempt.againstImpulse, true)
	// Порог из конфига: при 20% тот же ход уже не «против импульса».
	const [loose] = detectPoiConfirmation([makePoi()], ltf, htf, { impulseGatePct: 0.2 })
	assert.equal(loose!.attempts[0]!.againstImpulse, false)
	// Без HTF-данных пометка честно null.
	const [bare] = detectPoiConfirmation([makePoi()], ltf)
	assert.equal(bare!.attempts[0]!.impulseRet, null)
	assert.equal(bare!.attempts[0]!.againstImpulse, null)
})
