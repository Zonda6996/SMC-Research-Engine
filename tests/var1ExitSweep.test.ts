...
it('var1: BE после partial превращает пробой в выход по среднему входу (0 на остатке)', () => {
	...
	// без ADD: partial 25% на +10 при риске 12 -> +0.208R, остаток вышел в БУ (0)
	const t = replayVar1Trade(rows, tr(rows), 0, 1, { partialFrac: 0.25, breakeven: true, stopMult: 12, addOn: false })!
	assert.equal(t.outcome, 'Partial')
	assert.ok(Math.abs(t.grossR - 0.25 * (10 / 12)) < 1e-9)
})

it('var1: ADD = сплит-вход 50/50; стоп после добора стоит ровно -1R', () => {
	...
	// avgEntry 88, вес 1 (0.5+0.5): убыток (82-88)/90 / (0.75*8/90) = -1R
	assert.ok(Math.abs(t.grossR - -1) < 1e-9)
})

it('var1: ADD - стоп ДО добора стоит -0.667R (в рынке была половина размера)', () => {
	// вход 90, stopMult 8 -> стоп 82, add 86; бар прыгает сразу на стоп (гэп ниже add и стопа)
	const rows = [mk(0, 90, 91, 89, 90), mk(1, 90, 90, 90, 90), mk(2, 90, 90, 80, 80.5)]
	const t = replayVar1Trade(rows, tr(rows), 0, 1, { partialFrac: 0.25, breakeven: false, stopMult: 8, addOn: true })!
	assert.equal(t.outcome, 'Stop')
	// добор и стоп в одном баре: добор НЕ исполняется (advWick стопа тоже true), вес 0.5
	// убыток 0.5*(82-90)/90 / (0.75*8/90) = -0.6667R
	assert.ok(Math.abs(t.grossR - -(2 / 3)) < 1e-9)
})
