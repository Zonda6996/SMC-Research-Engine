...
it('tf profiles: all tfs group into wide bands, daily+ widest', () => {
	const h1 = heatmapConfigForTf(3_600_000)
	assert.equal(h1.minRelVolume, 1.0)
	assert.equal(h1.maxClusterBins, 6)
	assert.equal(h1.binPct, 0.005)
	const h4 = heatmapConfigForTf(14_400_000)
	assert.equal(h4.minRelVolume, 1.0)
	assert.equal(h4.binPct, 0.006)
	assert.equal(h4.maxClusterBins, 6)
