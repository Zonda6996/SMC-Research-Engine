...
**Внешний ревью (2026-08-24) — проверка честности линии Doppler.** Независимый агент: «подделок нет,
но GO ×3 завышен». Проверено по коду, согласовано частично: (1) формулировка «GO ×3» исправлена на
точную (2 preregistered GO с оговорками + in-sample мажоры + не-репликация tp + 0 живых сделок);
(2) гэп-исполнение стопа ИЗМЕРЕНО: гэп-пробоев 0/118 на мажорах −15/−5, конвенции идентичны
(`ci-results/d6-gapfill-robustness.*`) — претензия теоретически верна, эмпирически пуста (для Б/5м
не проверено); (3) OI берётся на ОТКРЫТИЕ бара i (последнее известное ≤10 мин) — причинно и
консервативнее, событие оценивается по закрытию i, вход на i+1 — расхождения с описанием нет;
(4) slippage отсутствует по замороженному стандарту автора (5bps/side = только комиссия); при
средней сделке +1.3…+3.1% проскальзывание десятков bps не меняет знак; (5) pooled maxDD в results
действительно НЕ хронологический (обход строк по символам) — heat-подобная метрика, на mean/CI не
влияет; для будущих раннеров — сортировать сделки по времени; (6) ATR-скип событий без счётчика —
подтверждён (1 шт в d6-tp), в будущих раннерах добавлять счётчик; (7) форвард торгует вариант с
reclaim-выходом, а «лучшая рука» — чистый H72-таймаут: расхождение осознанное (Reclaim — тоже
preregistered GO mgmt), решение об унификации — за автором.
