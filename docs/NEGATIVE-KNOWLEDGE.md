...
### Утечка #2 — `pool.notional` за всю жизнь пула (НЕ ИСПРАВЛЕНО)
**Где:** `LiquidityHeatmapEngine` считает `notional` пула суммой за весь срок жизни
(`startIndex..lastContributionIndex`). В `detectLiquidityPoi` на баре `t` в `freshPools`
попадают пулы с `startAt < t`, но `notional` берётся **полный** — включая объём после `t`.
Значит ранжирование полок (`shelfTopN`, `shelfMinShare`) и решение «рождать зону» на баре
`t` частично опираются на будущий объём. Числитель `stackNotional` тоже несёт #2.
**Масштаб:** мягче #1 — геометрия краёв (`bandLow/bandHigh`) причинна; загрязнён
отбор/тайминг рождения зон.
**Почему не чиним сейчас:** правка в калиброванном ядре POI (~285 тестов, визуальная
калибровка). Делать осознанно, отдельно, с QA. Подход: `notional-as-of-t` (частичная
сумма до бара `t`) вместо единого скаляра.
