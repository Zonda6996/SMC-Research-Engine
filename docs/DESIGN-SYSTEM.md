...
- green `#4ade80` — тейк/подтверждение (FULL).
- red `#f87171` — стоп (STOP).
- amber `#fbbf24` — частичка/безубыток (PARTIAL/BE).
- purple `#9a7bff` — тайм-стоп (TIME).
- cyan `#38bdf8` — вспомогательные акценты графика.
- muted `#a1a1a1` — служебные подписи.

В интерфейсе визуализатора используется русская локализация; названия компонентов выше — техническая карта, а не пользовательские заголовки.
...
- Панели: Overview, Filters, Heatmap, Zones (POI), Confirmation, Decision Lab, Config, Trades и Trade Detail.
