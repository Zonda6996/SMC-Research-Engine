...
## 0.5 Текущий фронт работ (25.07.2026): приёмка полной 15m-истории (Задание №3, часть 1); согласование МТФ (часть 2)
...
viz: полная 15m-история (Задание №3 ч.1) — tools/shared/archiveKlines.ts + склейка в server + чекбокс UI
docs: этот хэндофф
```

Версии движков: `liquidity-poi-2.4-consumed-at-close` (зоны НЕ менялись), `poi-confirmation-1.8-sweep-dedup`, `liquidity-heatmap-2.0-oi-hybrid`. Тесты 295/295 (`npx tsx --test tests/*.test.ts`), tsc чистый, node --check всех .mjs чистый. Дисциплина коммитов: движок и визуализатор — ОТДЕЛЬНЫМИ коммитами; merge в main — только по явной просьбе.
...
1. **Live-приёмка части 1** пользователем (его машина, его OI-карта): полная воронка на limit 5000, скрин = ТЗ; краевые случаи — символы с короткой историей, исторический срез until, «Пересчитать» без перекачки.
2. **Часть 2 — МТФ-подтверждение (детали согласовать с пользователем ДО кода)**: связки контекст→уточнённое по SPEC §14.1 (1W→4h, 1D→1h, 4h→15m текущая, 1h→5m); движки TF-агностичны (константы в барах ТФ + ATR), heatmap-профили 1D/1W есть — работа в основном server+UI (выбор conf-ТФ по лестнице, данные через archiveKlines, подписи). Решения пользователя: какие связки первыми (кандидаты 1D→1h и 1h→5m), семантика констант (96 баров idle на 1h = 4 суток вместо суток — оставить в барах или пересогласовать), глубины по ТФ.
3. **Форвард-наблюдение пометок v1.8** («против импульса», «дубль входа») на OI-карте пользователя; гейт-фильтр — только после форвард-статистики.
4. **Упрощённый режим подтверждения** (GGI ZONE, закрытая свеча в сторону зоны на 1h/4h, БУ/частичная фиксация) — после МТФ.
...
- Проверки после правок: `npx tsx --test tests/*.test.ts` (295/295), `npx tsc --noEmit`, `node --check tools/visualizer/public/*.mjs tools/visualizer/public/{lib,panels}/*.mjs`.
