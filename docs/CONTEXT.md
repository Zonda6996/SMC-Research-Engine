...
## Открытый вопрос (последний шаг перед позиционированием)

**SPEC 7.26 — сравнение схем выхода** на каноне (touch+bigbar, косты BingX): full@100 vs full@141 vs 50%@141+50%@241(BE), раздельно ote/deep. Код в прогоне готов (`--eval-entry`, блок «Exit schemes» внизу вывода). **Пользователь должен прогнать локально и скинуть evalentry-*.txt** — цифры это закроют.

Отложенные идеи фильтров (после 7.26): «свежесть касания» (вход только если касание зоны в первые N свечей жизни сетки), «близость тейка» (пропуск, если на входе до тейка < X от стопа).
