...
## 0.5 Текущий фронт работ (25.07.2026, вечер): live-приёмка МТФ-связок (Задание №3, часть 2 реализована)
...
1. **Live-приёмка части 2 (МТФ)** пользователем: ТФ-кнопки 1d/1h строят зоны и подтверждение по лестнице (1d→1h, 1h→5m), подписи/воронка показывают conf-ТФ, чекбокс «подтверждение: вся история» работает на всех связках; скрин = ТЗ. Решения пользователя 25.07: связки 1D→1h + 1h→5m (1W→4h отложена), константы В БАРАХ без скейлинга, глубины автоматические.
