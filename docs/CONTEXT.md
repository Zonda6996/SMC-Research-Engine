...
## 0.5 Текущий фронт работ (26.07.2026): приёмка иерархии слоёв (§16.21) + сессия требований упрощённого режима
...
viz: ИЕРАРХИЯ слоёв (§16.21, одиннадцатый QA, три скрина = эталон) — per-TF профили зон в
     tools/shared/poiProfiles.ts (1h локальный: этажи ≤2%; 1d свинг: консолидация + топ-3 + minShare 0.12;
     4h = дефолты), чекбокс «зоны текущего ТФ», компактная отрисовка слоёв (правая треть, фокус полный)
docs: этот хэндофф
```
...
1. **Приёмка иерархии слоёв (§16.21)** по трём эталонным скринам пользователя (26.07, BTC): 1h-слой — этажи ≤2% у цены (лонг 63.4→62.1 ≈ эталон), 1d — консолидированные области (шорт 68.5→74.0 ≈ эталон 69.8→75.2), «зоны текущего ТФ» выключаются, слои рисуются компактно справа. Сверить на его OI-карте; скрин = ТЗ.
