...
> Обновлено: 2026-07-24 (после v2.2 по пятому визуальному QA). Ветка: `liquidity-improvements-v1`, HEAD — коммит «poi v2.2» поверх `7a8fbb7`. Актуальный фронт работ — раздел 0.5, читать его ПЕРВЫМ; правила движков — SPEC §16.8–§16.14.
>
...
На коммите `864895e` проходили 217/217 тестов. На текущей ветке (24.07.2026, коммит «poi v2.2») — 285/285 (`npx tsx --test tests/*.test.ts`), `tsc --noEmit` чистый, `node --check tools/visualizer/public/app.js` чистый.
...
## 0.5 Текущий фронт работ (24.07.2026): редизайн визуализатора + QA v2.2
...
поверх   v2.2: родство стеков (близнецы/поколения), сила полки stackShare + фильтр UI, перф-фиксы (SPEC §16.14)
```

Версии движков: `liquidity-poi-2.2-stack-kinship`, `poi-confirmation-1.6-quiet-reanchor` (подтверждение НЕ менялось). Тесты 285/285, `tsc --noEmit` чистый, `node --check tools/visualizer/public/app.js` чистый. Правила движков — SPEC §16.8–§16.14 поверх §12–§15.

### История калибровки (кратко; детали и цифры — SPEC §16.13/§16.14)
