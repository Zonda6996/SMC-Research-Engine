...
## 0.5 Текущий фронт работ (25.07.2026, ночь): live-приёмка СЛОЁВ карты (свинг 1D + локальные 1h, §16.20)
...
Версии движков: `liquidity-poi-2.4-consumed-at-close` (зоны НЕ менялись), `poi-confirmation-1.8-sweep-dedup`, `liquidity-heatmap-2.0-oi-hybrid`. Тесты 297/297 (`npx tsx --test tests/*.test.ts`), tsc чистый, node --check всех .mjs чистый. Дисциплина коммитов: движок и визуализатор — ОТДЕЛЬНЫМИ коммитами; merge в main — только по явной просьбе.
...
1. **Live-приёмка СЛОЁВ (§16.20)** пользователем на рабочем виде 4h: чекбоксы «слой 1D свинг» и «слой 1h локальные» в панели зон (бейджи ТФ в списке, префиксы на прямоугольниках), селектор связки в панели подтверждения (трейсы свинга на 1h-свечах, локальных на 5m), сводка слоёв в воронке Обзора. КЛЮЧЕВОЕ требование пользователя: «локальные ≠ мусорные» — если его OI-карта покажет шум на 1h-слое, калибровать фильтрами/overrides по его скринам, НЕ хардкодить. ТФ-кнопки 1d/1h остаются одиночными видами (§16.19); скрин = ТЗ.
...
- Проверки после правок: `npx tsx --test tests/*.test.ts` (297/297), `npx tsc --noEmit`, `node --check tools/visualizer/public/*.mjs tools/visualizer/public/{lib,panels}/*.mjs`.
