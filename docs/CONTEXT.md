...
> Обновлено: 2026-07-24 (после калибровки v2.1 по четвёртому визуальному QA). Ветка: `liquidity-improvements-v1`, HEAD — коммит «poi v2.1» поверх `1ac04e9`. Актуальный фронт работ — раздел 0.5, читать его ПЕРВЫМ; правила движков — SPEC §16.8–§16.13.
>
...
На коммите `864895e` проходили 217/217 тестов. На текущей ветке (24.07.2026, коммит «poi v2.1») — 284/284 (`npx tsx --test tests/*.test.ts`), `tsc --noEmit` чистый, `node --check tools/visualizer/public/app.js` чистый.
...
## 0.5 Текущий фронт работ (24.07.2026): визуальное QA калибровки v2.1 + редизайн визуализатора
...
1ac04e9  docs: диагноз 4-го QA + план калибровки
поверх   v2.1: полки из провалов плотности, потолок 8% цены, свежесть 300, гард дедупа 2× (SPEC §16.13)
```

Версии движков: `liquidity-poi-2.1-valley-shelves`, `poi-confirmation-1.6-quiet-reanchor` (подтверждение НЕ менялось). Тесты 284/284, `tsc --noEmit` чистый, `node --check tools/visualizer/public/app.js` чистый. Правила движков — SPEC §16.8–§16.13 поверх §12–§15.

### Что сделала калибровка v2.1 (24.07.2026, утверждена пользователем; детали и цифры — SPEC §16.13)

1. Диагноз 4-го QA воспроизведён на 5000×4h до 22.07 (data.binance.vision, volume-proxy): гиганты = потолок 6 ATR на мега-цепях склейки полос (189/132/101 пулов), in-play шорт прятал фильтр Fresh, P-столбы = pivotTimes полки.
2. Кластеризация по ядрам (план из прошлого чата) ПРОВЕРЕНА И ОТВЕРГНУТА: 0.25 ATR — 47 осколков волатильной полки, 0.5 — 42 осколка спокойной И мега-цепь 74.5–98k. Шаг лестницы ликвидаций — в % цены, не в ATR.
3. Реализовано: полка = супер-цепь (склейка §16.10), разрезанная по ПРОВАЛАМ ПЛОТНОСТИ (корзины 0.4% цены, провал = ≥3 корзины < 25% пика цепи); потолок stackMaxPct 8% от края (stackMaxAtr удалён); shelfFreshBars 300; shelfMinShare 0.03; дедуп-гард dupMaxHeightRatio 2.0 (выросшая >2× полка не дубль старой узкой); LiquidityPoiContext.config для переопределения констант.
4. UI-фиксы: без P-маркеров у liquidity-shelf, дефолт фильтра «Открытые — готова или в игре», справка панели зон переписана.
5. Карта против эталона пользователя: 4 из 5 ручных зон на месте (ближайшие long/short почти точные), известный остаток — цепочка поколений дедупа с шагом ≤2× маскирует верх выросшей полки 55.5–58k (57446→53197 — дубль промежуточной 55515→52545, ratio 1.43).

### ГЛАВНОЕ ЗАДАНИЕ №1: визуальное QA v2.1 по скринам пользователя

Пользователь смотрит визуализатор (4h, limit 5000, у него heatmap с OI-гибридом — карта может отличаться от volume-proxy песочницы в свежих пулах) и присылает скрины. По каждому кейсу: воспроизвести на данных до правок → диагноз → предложение → согласование → реализация. Особо проверить: (а) остаток дедупа на 55.5–58k — если пользователю мешает, кандидаты: перекрытие относительно МЛАДШЕЙ зоны вместо меньшей (без новой константы) или транзитивный гард; (б) мелкие фоновые зоны (62.9–61.8, 58.9–58.2) — шум или полезны; (в) near шорта 71718→75233 против ручного 72.8 (смещение на volume-proxy?); (г) не слишком ли много активных зон (14).

### ГЛАВНОЕ ЗАДАНИЕ №2: полный редизайн визуализатора (утверждено, после приёмки калибровки)

Стиль shadcn/ui + Vercel: строгий, минималистичный; их компоненты МОЖНО. Больше настроек (конфиги движков в UI — бэкенд уже готов через LiquidityPoiContext.config; для heatmap конфиг передаётся аргументом detectLiquidityHeatmap). Зоны рисовать ПРЯМОУГОЛЬНИКАМИ с заливкой (custom primitive в lightweight-charts v5), не парой линий. Сохранить обязательно: запуск `npx tsx tools/visualizer/server.ts` (порт 7788), без автозагрузки (кнопка «Загрузить»), панели Heatmap / Liquidity POI / POI Confirmation / Decision Lab, кнопка «Зоны на 4h», русские подписи и пояснения терминов, `node --check` для JS фронта. Решение по сборке ОТЛОЖЕНО пользователем до приёмки калибровки: вариант A — ванильный JS/CSS с дизайн-системой в духе shadcn (без сборки, рекомендация исполнителя), вариант B — React+Tailwind+shadcn с закоммиченным dist. Не сломать «склонировал и запустил».
