...
### ГЛАВНОЕ ЗАДАНИЕ №1: калибровка Liquidity POI 2.0 по визуальному QA

ЯДРО v2.0 РЕАЛИЗОВАНО и запушено (см. статус в SPEC §16.12): движок `liquidity-poi-2.0-liquidity-first` строит зоны от полок ликвидности; карта на 5000 барах уже близка к ручной карте пользователя (лонги 59.2–61k / 61.5–63.6k, шорт 65.6–74.1k над ценой). Текущий фронт — калибровка ручек (shelfFreshBars/TopN/MinShare/GapAtr — список в SPEC §16.12) по скринам пользователя + пометка «на структуре» и объёмная пометка прихода в UI. Исходное ТЗ ниже — для контекста.
