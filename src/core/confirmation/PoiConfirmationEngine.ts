...
//  - потеря защиты переносит якорь на новый экстремум (ANCHOR_DEEPENED); v1.6 (§16.11): после переноса
//    подтверждение строится ЗАНОВО — проторговка у нового якоря + отскок (входы «одним движением» убраны);
	...
	for (let j = touch; j < ltf.length; j++) {
		...
		if (stage === 'protect') {
			...
			if (!protectOn(j, c) && belowCloses >= cfg.failedProtectionCloses) {
				...
				// §16.11: потеря защиты внутри зоны — якорь переносится на новый экстремум («этот фитиль
				// становится тем хаем, который надо пересвипнуть»), и подтверждение строится заново:
				// проторговка у нового якоря + отскок. Мгновенный пересвип без отката давал входы
				// «одним движением» (QA скрин 2); смерть по будильнику это больше не вернёт —
				// idle-таймер и доигрывание за окном (v1.9) держат попытку живой до реального пересвипа.
				low = sweepExtreme
				stage = 'stopping'
				lastExtremeBar = j
				stoppingBar = -1
