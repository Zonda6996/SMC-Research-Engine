...
/**
 * Bigbar-фильтр (идея пользователя): true, если ОДНА свеча телом перекрыла
 * всю входную зону [zoneNear, zoneFar]. Уровень «сожжён» — сетап
 * пропускается. Окно: от создания сетки ДО свечи касания ВКЛЮЧИТЕЛЬНО —
 * пользовательский сценарий «свеча тянется от 100 до 78» — это чаще всего
 * сама свеча, дошедшая до зоны. Для confirm-моделей тело свечи касания
 * известно в момент решения (вход по закрытию) — не look-ahead; для
 * touch-модели (лимитка) метка диагностическая.
 *
 * Первая версия исключала свечу касания и дала 0 срабатываний на пуле
 * 6537 (до касания цена ещё не была в зоне — накрыть её телом нечем).
 */
