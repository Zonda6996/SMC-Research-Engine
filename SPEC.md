...
**Первое: фид другой.** Мой бар (Binance USDT-M): 1h O 64970.8 H 64982 L 64571 C 64627.2; 15m O 64970.8 H 64982 L 64668.1 C 64729.6. Расхождение ~30 пунктов (0.05%) — пользователь смотрит Bybit. Для 200-периодных величин это шум, но точное совпадение до копейки на наших данных недостижимо в принципе.
...
**Задача поставлена численно: подобрать (мера × период) так, чтобы одновременно попасть в 240.13 на 1h и 176.16 на 15m при mean = WMA(200) от hlc3.** Четыре опорных точки уже есть (2 ТФ × 2 даты). Пока не решено — полосы в визуализаторе показывать с пометкой «калибровка не завершена».
...
2. **Сигнал может появиться на незакрытом баре и исчезнуть по закрытию.** Значит индикатор считается вживую, а окончательное состояние определяется закрытием бара — постфактум он не рисует, look-ahead нет.
...
- полосы перекупленности/перепроданности → **«Растяжка» / StretchBands** (альтернативы: «Прилив»/TideBands, «Канал давления»/PressureChannel);
- сигнальный индикатор → **«Разворот» / ReversalTag** (альтернативы: «Отскок»/ReboundSignal, «Метка края»/EdgeMark).
