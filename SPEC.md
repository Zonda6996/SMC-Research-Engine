...
```text
liquidity-poi-1.4-overlap-dedup
```
...
### Параметры к калибровке по QA (все в LIQUIDITY_POI_CONFIG)

топ-N полок на сторону (старт 3) или доля notional; окно свежести (старт 500 баров); допуск near-экстремума от края полки (старт 0.5 ATR); порог «стек снят» (старт 50% remainingNotional); потолок ширины stackMaxAtr 6 ATR — остаётся.
