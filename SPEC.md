...
- TF profiles (engine default when no explicit config is passed; timeframe inferred from median candle spacing): sub-4h -> minRelVolume 1.0, maxClusterBins 5 (harder grouping of local noise); 1d and above -> minRelVolume 1.25, binPct 0.008, maxClusterBins 5 (wider bins so macro history is not a wall of thin bands); 4h keeps the base config. Version `liquidity-heatmap-1.2-tf-profiles-full-depth`;
- full-depth output: the engine no longer trims pools by whole-history rank (minWeight default 0.05 -> 0, maxPools 2000 -> 10000 as a pure payload safety cap). Ranking by the entire loaded history starved the recent window: with 15k 1h candles the top-2000 slots went to fat 2024 pools and the fresh window drew ~3x fewer bands than with a 5k load. The only visual cuts are renderer-side: per-window rank threshold and the top-400 draw cap;
