...
export function plannedFullStop(entry: number, stop: number, entryRate = BINGX_MAKER_RATE): { stopPct: number; costR: number; netR: number } {
	const risk = Math.abs(entry - stop)
	if (!(risk > 0) || !(entry > 0) || !(stop > 0)) {
		return { stopPct: 0, costR: Number.POSITIVE_INFINITY, netR: Number.NEGATIVE_INFINITY }
	}
	const costR = fillCostR(entry, entryRate, 1, risk) +
