// D1 — exit × vol-veto (трек D из docs/ROADMAP.md). Пред-регистрированный прогон.
//
// Гипотеза/мотив: baseline даёт vendor-WR ~0.83 при meanR≈0 → утечка R в геометрии ВЫХОДА (§8 NEGATIVE-KNOWLEDGE),
//   а из D4b известен durable причинный risk-filter «не торговать сигнал в low-vol режиме BTC-2h».
//   Здесь фиксируем вход (весь допущенный набор стрелок, filter=off, A1-путь) и сравниваем 4 плеча:
//     exit ∈ {A0 = safe/static/T2 (референс), A1 = pure-hold (без фикс-TP, выход по стопу/maxHoldingBars)}
//     veto ∈ {OFF = high∪low, ON = только high-vol}   ⇒ 2×2 = 4 плеча.
//   Меряем train/OOS 65-35 по времени + bootstrap CI + per-asset breadth (OOS).
//
// §2.1: НИ ОДНОГО нового числа. A0 — replayStatic ВЕРБАТИМ из regimeGateD4Vol.ts (safe, static-full, TARGET_STEPS=2).
//   A1 — та же функция БЕЗ take-profit-ветки (убрана ветка favorableWick(staticFull) и переменная staticFull);
//   выход только по стопу или maxHoldingBars (timeout) — новых порогов нет, это аблация выхода.
//   Vol-режим — причинный BTC-2h realized-vol (окна VOL_WINDOW=120 + VOL_MEDIAN_WINDOW=1000, заранее заданы, НЕ свипаются).
// §2.3: src не тронут; replayStatic, vol-режим, regimeAsOf и stats-хелперы скопированы вербатим из regimeGateD4Vol.ts.
// Данные — офлайн кэш. Запуск: npx tsx tools/research/exitVetoD1.ts.
...
// ⚠ окна режима — заранее заданные нейтральные прокси, НЕ свипаются (§2.1). Совпадают с regimeGateD4Vol.ts.
...
// —— A0: replayStatic ВЕРБАТИМ из regimeGateD4Vol.ts (safe/static/T2) ——
...
// —— A1: pure-hold — та же логика БЕЗ take-profit-ветки (нет staticFull). Выход только по стопу или maxHoldingBars. ——
...
// —— stats helpers ВЕРБАТИМ из regimeGateD4Vol.ts ——
...
// —— causal BTC-2h realized-vol регим ВЕРБАТИМ из regimeGateD4Vol.ts ——
...
// —— сбор пула сделок для A0 и A1 (общий вход, общий vol-режим) ——
...
// warmup (volRegime==null) исключаем из ОБОИХ veto, чтобы единственное отличие OFF↔ON было low-vol дроп.
...
console.log(`\n===== D1 exit × vol-veto (BTC-2h realized-vol) =====`)
console.log(`BTC-2h баров: ${btcBarsLoaded}; VOL_WINDOW=${VOL_WINDOW}; VOL_MEDIAN_WINDOW=${VOL_MEDIAN_WINDOW}; high=${btcHighBars} low=${btcLowBars}`)
...
console.log(`\n----- per-asset breadth (OOS) по плечам -----`)
for (const exit of EXITS) for (const veto of VETOS) {
	const key = `${exit}/${veto}`
	console.log(`  ${key}:`)
...
const result = {
	...
	protocol: 'D1-exit-veto-1.0 (pre-registered: A0 safe/static/T2 vs A1 pure-hold; veto ON=high-vol-only vs OFF; filter=off, A1 admission path)',
	preregistration: {
		universe: ASSETS,
		timeframes: TIMEFRAMES,
		geometry: GEO,
		exits: {
			A0: 'safe/static-full, target 2×step (референс, replayStatic verbatim)',
			A1: 'pure-hold: без фикс-TP, выход только по стопу или maxHoldingBars (timeout)',
		},
		vetoDefinition: 'ON = только high-vol (low-vol дроп, durable risk-filter из D4b); OFF = high∪low; warmup (regime=null) исключён из обоих',
		volWindow: VOL_WINDOW,
		medianWindow: VOL_MEDIAN_WINDOW,
		trainFraction: TRAIN_FRACTION,
		costs: '7 bps/side (ARROW_MODE_CONFIGS.oneWayCostBps); funding omitted',
		bootstrap: `${BOOTSTRAP_SAMPLES} trade-level resamples, seed ${BOOTSTRAP_SEED}`,
		note: '§2.1: ни одного нового числа (A1 = аблация TP-ветки A0; окна волы заранее заданы, НЕ свипаются). §2.3: src не тронут, replay/vol/helpers вербатим из regimeGateD4Vol.ts. Режим строго причинный.',
	},
	sources,
	skipped,
	btcBarsLoaded, btcHighBars, btcLowBars,
	warmupExcluded,
	arms,
}
writeFileSync(resolve('ci-results/exit-veto-d1.json'), JSON.stringify(result, null, 2))
console.log(`\nWrote ci-results/exit-veto-d1.json`)
