...
const CONF_FIELDS = {
	attemptIdleBars: ['Смерть по бездействию, баров', 'Столько баров ТФ подтверждения без структурных событий — попытка умирает (timeout@…). Больше — попытки живут дольше'],
