...
function hmMergePools(pools,gap){if(!gap)return pools;const out=[];for(const s of['sell-side','buy-side']){const list=pools.filter(p=>p.side===s).sort((a,b)=>a.bandLow-b.bandLow);let cur=null;for(const p of list){if(cur&&p.bandLow<=cur.bandHigh*(1+gap)){cur.bandHigh=Math.max(cur.bandHigh,p.bandHigh);cur.notional+=p.notional;cur._mid+=p.extremePrice*p.notional;cur.startAt=Math.min(cur.startAt,p.startAt);cur.merged++;if(p.status==='active'){cur.status='active';cur.sweptAt=null}else if(cur.status!=='active')cur.sweptAt=Math.max(cur.sweptAt??0,p.sweptAt??0)}else{cur={...p,_mid:p.extremePrice*p.notional,merged:1};out.push(cur)}}}for(const m of out)if(m.merged>1)m.extremePrice=m._mid/m.notional;return out}
