...
function renderDetail(){
	...
	$('detail').innerHTML=`<div class="detail-grid"><div><span class="label">Поток:</span> ${t.stream.toUpperCase()}${t.shadow?' · SHADOW':''}</div><div><span class="label">Исход:</span> <b class="${cls(t.netR)}">${t.result} ${fmtR(t.netR)}</b></div><div><span class="label">Направление:</span> ${t.direction}</div><div><span class="label">Триггер:</span> ${t.trigger.toUpperCase()}</div><div><span class="label">Entry:</span> ${fmtP(t.entry)}</div><div><span class="label">Stop:</span> ${fmtP(t.stop)}</div><div><span class="label">Take:</span> ${fmtP(t.take)}</div><div><span class="label">Exit:</span> ${fmtP(t.exitPrice)}</div><div><span class="label">Fresh:</span> ${t.freshBars} бар.</div><div><span class="label">Hold:</span> ${t.holdBars??'open'} бар.</div><div><span class="label">Risk:</span> ${t.shadow?'0 (shadow)':`x${t.riskMult}`}</div><div><span class="label">Bigbar:</span> ${t.bigbarDiagnostic?'ДА, диагностический':'нет'}</div><div><span class="label">Fib 0%:</span> ${fmtP(t.legStart.price)}</div><div><span class="label">Fib 100%:</span> ${fmtP(t.legEnd.price)}</div></div>${t.shadow?'<div class="notice">Mirror только наблюдается. В боевой R и размер позиции не входит.</div>':''}`
}
