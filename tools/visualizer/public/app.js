...
function renderHeatmap(){if(!data)return;if(!hmOn){$('hmStatus').textContent='\u2014';return}const{pools}=hmPools(),minW=Number($('hmMinWeight').value),last=data.candles[data.candles.length-1].timestamp,dw=new Map();for(const s of['sell-side','buy-side']){const list=pools.filter(p=>p.side===s).sort((a,b)=>a.notional-b.notional);list.forEach((p,i)=>dw.set(p,(i+1)/list.length))}const shown=pools.filter(p=>dw.get(p)>=minW).sort((a,b)=>dw.get(a)-dw.get(b)).slice(-400);for(const p of shown){const w=dw.get(p),alpha=(0.1+0.75*w*w).toFixed(2),width=Math.max(2,Math.min(8,2+Math.round(6*Math.pow(w,1.5))));line([{time:time(p.startAt),value:p.extremePrice},{time:time(p.sweptAt??last),value:p.extremePrice}],{color:`rgba(${p.side==='sell-side'?'239,83,96':'34,197,143'},${alpha})`,lineWidth:width})}const act=pools.filter(p=>p.status==='active').length;$('hmStatus').textContent=`\u041f\u0443\u043b\u044b: ${act} \u0430\u043a\u0442\u0438\u0432\u043d\u044b\u0445 \u00b7 ${pools.length-act} \u0441\u043d\u044f\u0442\u044b\u0445 \u00b7 \u043d\u0430\u0440\u0438\u0441\u043e\u0432\u0430\u043d\u043e ${shown.length} \u0438\u0437 ${pools.length}`}
