...
function hmPools(){const side=$('hmSide').value,minW=Number($('hmMinWeight').value),showSwept=$('hmShowSwept').checked,age=Number($('hmAge').value);let cutoff=-Infinity;if(age>0&&data.candles.length>age)cutoff=data.candles[data.candles.length-age].timestamp;return(data?.liquidityHeatmap?.pools||[]).filter(p=>(side==='all'||p.side===side)&&p.weight>=minW&&(showSwept||p.status==='active')&&(p.sweptAt??p.lastContributionAt??p.startAt)>=cutoff)}
function renderHeatmap(){if(!data)return;if(!hmOn){$('hmStatus').textContent='\u2014';return}const xs=hmPools(),ordered=[...xs].sort((a,b)=>b.weight-a.weight),cap=400,shown=ordered.slice(0,cap),last=data.candles[data.candles.length-1].timestamp;for(const p of shown){const w=p.weight,sell=p.side==='sell-side',c0=sell?[150,75,85]:[70,120,100],c1=sell?[255,85,100]:[55,225,160],cr=Math.round(c0[0]+w*(c1[0]-c0[0])),cg=Math.round(c0[1]+w*(c1[1]-c0[1])),cb=Math.round(c0[2]+w*(c1[2]-c0[2])),alpha=(0.3+0.6*w).toFixed(2),width=Math.max(1,Math.min(9,1+(p.spanBins||1)+Math.round(5*w)));line([{time:time(p.startAt),value:p.extremePrice},{time:time(p.sweptAt??last),value:p.extremePrice}],{color:`rgba(${cr},${cg},${cb},${alpha})`,lineWidth:width})}const act=xs.filter(p=>p.status==='active').length;$('hmStatus').textContent=`\u041f\u0443\u043b\u044b: ${act} \u0430\u043a\u0442\u0438\u0432\u043d\u044b\u0445 \u00b7 ${xs.length-act} \u0441\u043d\u044f\u0442\u044b\u0445 \u00b7 \u043d\u0430\u0440\u0438\u0441\u043e\u0432\u0430\u043d\u043e ${shown.length}${ordered.length>cap?` \u0438\u0437 ${ordered.length}`:''}`}
