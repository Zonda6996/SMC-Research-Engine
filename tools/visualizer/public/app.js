...
function toggleLab(){labMode=!labMode;$('labControls').style.display=labMode?'block':'none';$('labToggle').textContent=labMode?'Выключить':'Включить';$('labToggle').classList.toggle('active',labMode);if(labMode){refinedMode=false;poiZoneMode=false;confMode=false;selectedId=null;rebuildLabOrder();setLabCursor(currentLab());renderLab()}else{document.body.classList.remove('lab-blind');chart.applyOptions({timeScale:{visible:true},rightPriceScale:{visible:true}});restoreMainCandles();redraw();chart.timeScale().fitContent()}}
...
function renderPoiZones(){if(!data||!poiZoneMode)return;clearOverlays();restoreMainCandles();markersPlugin.setMarkers([]);const all=poiZoneCandidates(),xs=all.slice(0,80);renderPoiZoneList(xs);const last=data.candles[data.candles.length-1].timestamp,marks=[];for(const c of xs){const focused=c.id===poiFocusId,dim=poiFocusId&&!focused,color=c.direction==='long'?C.green:C.red,end=time(c.endAt||last),from=time(c.originAt);line([{time:from,value:c.near},{time:end,value:c.near}],{color,lineWidth:focused?3:dim?1:2});line([{time:from,value:c.far},{time:end,value:c.far}],{color:C.purple,lineWidth:focused?3:dim?1:1,lineStyle:LightweightCharts.LineStyle.Dashed});if(focused)for(const at of c.pivotTimes||[])marks.push({time:time(at),position:c.direction==='long'?'belowBar':'aboveBar',color:C.purple,shape:'circle',size:1,text:'P'})}markersPlugin.setMarkers(marks.sort((a,b)=>a.time-b.time));$('poiZoneStatus').textContent=`Показано зон: ${xs.length}${all.length>xs.length?` из ${all.length}`:''} (всего в наборе ${data?.liquidityPoi?.candidates?.length||0})`;const c=xs.find(x=>x.id===poiFocusId);if(c){$('poiZoneDetail').innerHTML=`<div><b>Зона:</b> ${fmtP(c.near)} → ${fmtP(c.far)} · ${c.zoneClass}</div><div>Priority: ${c.priority} · lifecycle: ${c.lifecycleState} · interaction: ${c.interaction} (${c.touchCount||0})</div><div>P/D: ${c.pdZone||'none'} · aligned: ${c.pdAligned==null?'unknown':c.pdAligned?'yes':'no'} · boundary: ${c.boundarySource}</div><div>Жизнь: ${new Date(c.knownAt).toLocaleString('ru-RU')} → ${c.endAt?new Date(c.endAt).toLocaleString('ru-RU'):'сейчас'}</div><div>Armed: ${c.armedAt?new Date(c.armedAt).toLocaleString('ru-RU'):'—'} · consumed: ${c.consumedAt?new Date(c.consumedAt).toLocaleString('ru-RU'):'—'} · failed: ${c.failedAt?new Date(c.failedAt).toLocaleString('ru-RU'):'—'} · retired: ${c.retiredAt?new Date(c.retiredAt).toLocaleString('ru-RU'):'—'}</div>`;chart.timeScale().setVisibleRange({from:time(c.originAt-20*14_400_000),to:time((c.endAt||last)+20*14_400_000)})}else{$('poiZoneDetail').innerHTML='';chart.timeScale().fitContent()}}
...
function togglePoiZones(){poiZoneMode=!poiZoneMode;$('poiZoneControls').style.display=poiZoneMode?'block':'none';$('poiZoneToggle').textContent=poiZoneMode?'Закрыть':'Открыть';if(poiZoneMode){refinedMode=false;labMode=false;confMode=false;poiFocusId=null;renderPoiZones()}else{restoreMainCandles();redraw()}}
...
function renderConfirmation(){if(!data||!confMode)return;clearOverlays();markersPlugin.setMarkers([]);const c=currentConfirmation(),xs=confirmationCandidates();if(!c){const total=confirmationAttempts().length;$('confStatusText').textContent=total?`Всего попыток ${total}, но по текущему фильтру 0 — выберите «Все попытки»`:'poiConfirmation: 0 попыток (нет POI-зон на 4h или пуст ltf15m)';return}const src=data.ltf15m||[];if(!src.length)return;candlesSeries.setData(src.map(x=>({time:time(x.timestamp),open:x.open,high:x.high,low:x.low,close:x.close})));const from=time(src[0].timestamp),to=time(src.at(-1).timestamp),zc=c.direction==='long'?C.green:C.red;line([{time:from,value:c.near},{time:to,value:c.near}],{color:zc,lineWidth:2});line([{time:from,value:c.far},{time:to,value:c.far}],{color:C.purple,lineWidth:2,lineStyle:LightweightCharts.LineStyle.Dashed});if(c.entry!=null&&c.stop!=null&&c.tp2!=null){line([{time:time(c.entryAt),value:c.entry},{time:to,value:c.entry}],{color:C.blue,lineWidth:3});line([{time:time(c.entryAt),value:c.stop},{time:to,value:c.stop}],{color:C.red,lineWidth:3});line([{time:time(c.entryAt),value:c.tp2},{time:to,value:c.tp2}],{color:C.green,lineWidth:3})}const colors={POI_TOUCH:C.blue,STOP_CONFIRMED:C.amber,REBOUND:C.blue,SECOND_SWEEP:C.red,PROTECTED:C.green,LOW_VOLUME_TEST:C.dim,ENTRY:C.green,STOP:C.red,TP2:C.green};const marks=c.trace.map(x=>({time:time(x.at),position:['SECOND_SWEEP','STOP'].includes(x.state)?'belowBar':'aboveBar',color:colors[x.state]||C.dim,shape:x.state==='ENTRY'?'arrowUp':'circle',size:1,text:x.state})).filter(x=>src.some(s=>time(s.timestamp)===x.time));markersPlugin.setMarkers(marks.sort((a,b)=>a.time-b.time));$('confStatusText').textContent=`${confIndex+1}/${xs.length} · ${c.direction.toUpperCase()} · ${c.zoneClass.toUpperCase()} · попытка ${c.attemptIndex} · ${c.status.toUpperCase()}${c.outcome?' · '+c.outcome.toUpperCase():''} · ${c.rejectionReason||fmtR(c.grossR)}`;$('confTrace').innerHTML=`<div>Зона ${fmtP(Math.min(c.near,c.far))}–${fmtP(Math.max(c.near,c.far))}</div>${c.entry!=null?`<div>Entry ${fmtP(c.entry)} · Stop ${fmtP(c.stop)} · TP2 ${fmtP(c.tp2)} · ${fmtR(c.grossR)}</div>`:''}<hr>${c.trace.map(x=>`<div><b>${x.state}</b> · ${new Date(x.at).toLocaleString('ru-RU')}${x.volumeRatio!=null?` · vol×${x.volumeRatio.toFixed(2)}`:''}</div>`).join('')}`;const times=c.trace.map(x=>x.at);chart.timeScale().setVisibleRange({from:time(Math.min(...times)-6*3600000),to:time(Math.max(...times)+6*3600000)})}
function toggleConfirmation(){confMode=!confMode;$('confControls').style.display=confMode?'block':'none';$('confToggle').textContent=confMode?'Закрыть':'Открыть';if(confMode){refinedMode=false;labMode=false;poiZoneMode=false;confIndex=0;const reasons=[...new Set(confirmationAttempts().map(x=>x.rejectionReason).filter(Boolean))];$('confReason').innerHTML='<option value="all">Все причины</option>'+reasons.map(x=>`<option value="${x}">${x}</option>`).join('');renderConfirmation()}else{restoreMainCandles();redraw()}}
...
function toggleRefined(){refinedMode=!refinedMode;$('refinedControls').style.display=refinedMode?'block':'none';$('refinedToggle').textContent=refinedMode?'Закрыть':'Открыть';if(refinedMode){labMode=false;poiZoneMode=false;confMode=false;refinedIndex=0;renderRefined()}else{restoreMainCandles();redraw()}}
...
function redraw(){if(!data)return;if(poiZoneMode){renderPoiZones();return}if(confMode){renderConfirmation();return}if(refinedMode){renderRefined();return}if(labMode){renderLab();return}clearOverlays();renderList();renderEvents();renderProtected();renderMarkers();renderSelected();renderHeatmap();renderDetail();renderCards()}
