...
function drawHmProfile(){const cv=$('hmProfile');if(!cv)return;const wrap=cv.parentElement,W=110,H=(wrap&&wrap.clientHeight)||300;if(cv.width!==W)cv.width=W;if(cv.height!==H)cv.height=H;const ctx=cv.getContext('2d');ctx.clearRect(0,0,W,H);if(!hmOn||!hmShownBands.length||!candlesSeries||!chart)return;try{cv.style.right=`${chart.priceScale('right').width()||56}px`}catch{}let maxN=0;for(const x of hmShownBands)maxN=Math.max(maxN,hmNotional(x.p));for(const x of hmShownBands){const p=x.p,y1=candlesSeries.priceToCoordinate(p.bandHigh),y2=candlesSeries.priceToCoordinate(p.bandLow);if(y1==null||y2==null)continue;const h=Math.max(2,Math.abs(y2-y1)),len=Math.max(3,W*0.95*Math.sqrt(hmNotional(p)/(maxN||1)));ctx.fillStyle=`rgba(RED,ALPHA)`.replace('RED',p.side==='sell-side'?'239,83,96':'34,197,143').replace('ALPHA',(0.25+0.6*x.w*x.w).toFixed(2));ctx.fillRect(W-len,Math.min(y1,y2),len,h)}}
