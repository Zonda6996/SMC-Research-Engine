...
function liqCandidates(){const d=$('liqDirection').value,life=$('liqLifecycle').value;return(data?.liquidityPoi?.candidates||[]).filter(x=>(d==='all'||x.direction===d)&&(life==='all'||x.active))}
...
function saveLiq(choice){const c=currentLiq();if(!c)return;const x=liqReviews();x[c.id]={id:c.id,version:c.version,symbol:data.dataset.symbol,direction:c.direction,zoneClass:c.zoneClass,choice,note:$('liqNote').value.trim(),near:c.near,far:c.far,atr:c.atr,pivotCount:c.pivotCount,eventType:c.eventType,active:c.active,supersededAt:c.supersededAt,invalidatedAt:c.invalidatedAt,recordedAt:new Date().toISOString()};localStorage.setItem(LIQ_REVIEW_KEY,JSON.stringify(x));moveLiq(1)}
