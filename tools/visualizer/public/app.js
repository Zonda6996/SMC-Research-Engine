...
function hmPools(){const side=$('hmSide').value,minW=Number($('hmMinWeight').value),showSwept=$('hmShowSwept').checked;return(data?.liquidityHeatmap?.pools||[]).filter(p=>(side==='all'||p.side===side)&&p.weight>=minW&&(showSwept||p.status==='active'))}
function renderHeatmap(){if(!data)return;if(!hmOn){$('hmStatus').textContent='\u2014';return}const xs=hmPools(),ordered=[...xs].sort((a,b)=>b.weight-a.weight),cap=400,shown=ordered.slice(0,cap),last=data.candles[data.candles.length-1].timestamp;for(const p of shown){const rgb=p.side==='sell-side'?'255,102,117':'53,197,154',alpha=(0.22+0.68*p.weight).toFixed(2),width=Math.max(1,Math.min(4,1+Math.round(3*p.weight)));line([{time:time(p.startAt),value:p.extremePrice},{time:time(p.sweptAt??last),value:p.extremePrice}],{color:`rgba(${rgb},${alpha})`,lineWidth:width})}const act=xs.filter(p=>p.status==='active').length;$('hmStatus').textContent=`\u041f\u0443\u043b\u044b: ${act} \u0430\u043a\u0442\u0438\u0432\u043d\u044b\u0445 \u00b7 ${xs.length-act} \u0441\u043d\u044f\u0442\u044b\u0445 \u00b7 \u043d\u0430\u0440\u0438\u0441\u043e\u0432\u0430\u043d\u043e ${shown.length}${ordered.length>cap?` \u0438\u0437 ${ordered.length}`:''}`}
$('hmToggle').onclick=()=>{hmOn=!hmOn;$('hmControls').style.display=hmOn?'block':'none';$('hmToggle').textContent=hmOn?'\u0421\u043a\u0440\u044b\u0442\u044c':'\u041f\u043e\u043a\u0430\u0437\u0430\u0442\u044c';redraw()}
for(const id of['hmSide','hmMinWeight','hmShowSwept'])$(id).onchange=()=>redraw()
