...
function poiZoneCandidates(){const d=$('poiDirection').value,life=$('poiLifecycle').value,zc=$('poiZoneClass').value,pr=$('poiPriority').value,activeOnly=$('poiActiveOnly').checked,liqOnly=$('poiLiqOnly').checked;return(data?.liquidityPoi?.candidates||[]).filter(x=>{if(liqOnly&&x.boundarySource!=='liquidity-cluster')return false;if(activeOnly&&!(x.active&&x.valid))return false;if(d!=='all'&&x.direction!==d)return false;if(life!=='all'&&x.lifecycleState!==life)return false;if(zc!=='all'&&x.zoneClass!==zc)return false;if(pr!=='all'&&x.priority!==pr)return false;return true}).sort((a,b)=>b.knownAt-a.knownAt)}
