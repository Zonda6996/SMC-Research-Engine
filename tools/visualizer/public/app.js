...
function renderPoiZoneList(xs){const box=$('poiZoneList');box.innerHTML='';if(!xs.length){box.innerHTML='<div class="empty">Нет зон по фильтрам</div>';return}for(const c of xs){const el=document.createElement('div');el.className='trade'+(c.id===poiFocusId?' selected':'');el.innerHTML=`<span class="pill ${c.direction}">${c.direction.toUpperCase()}</span><span class="stream">${c.zoneClass.replace('-',' ').toUpperCase()}</span><span><span class="meta">${fmtP(c.near)} → ${fmtP(c.far)}</span></span><span class="result">${c.lifecycleState.toUpperCase()}</span>`;el.onclick=()=>{poiFocusId=poiFocusId===c.id?null:c.id;renderPoiZones()};box.appendChild(el)}}
function renderPoiZones(){if(!data||!poiZoneMode)return;clearOverlays();restoreMainCandles();markersPlugin.setMarkers([]);const all=poiZoneCandidates(),xs=all.slice(0,80);renderPoiZoneList(xs);const last=data.candles[data.candles.length-1].timestamp,marks=[];for(const c of xs){const focused=c.id===poiFocusId,dim=poiFocusId&&!focused,color=c.direction==='long'?C.green:C.red,end=time(c.endAt||last),from=time(c.originAt);const nearLine=line([{time:from,value:c.near},{time:end,value:c.near}],{color,lineWidth:focused?3:dim?1:2});line([{time:from,value:c.far},{time:end,value:c.far}],{color:C.purple,lineWidth:focused?3:dim?1:1,lineStyle:LightweightCharts.LineStyle.Dashed});if(focused){const known=Math.max(from,time(c.knownAt));LightweightCharts.createSeriesMarkers(nearLine,[{time:from,position:'inBar',color,shape:'circle',size:0,text:`NEAR ${fmtP(c.near)} · ${c.zoneClass.toUpperCase()}`},{time:known,position:c.direction==='long'?'belowBar':'aboveBar',color:C.text,shape:'circle',size:1,text:'ЗОНА ИЗВЕСТНА'}]);for(const at of c.pivotTimes||[])marks.push({time:time(at),position:c.direction==='long'?'belowBar':'aboveBar',color:C.purple,shape:'circle',size:1,text:'P'})}}markersPlugin.setMarkers(marks.sort((a,b)=>a.time-b.time));renderHeatmap();$('poiZoneStatus').textContent=`Показано зон: ${xs.length}${all.length>xs.length?` из ${all.length}`:''} (всего в наборе ${data?.liquidityPoi?.candidates?.length||0})`;const c=xs.find(x=>x.id===poiFocusId);if(c){$('poiZoneDetail').innerHTML=`<div><b>Зона:</b> ${fmtP(c.near)} → ${fmtP(c.far)} · ${c.zoneClass}</div><div>Priority: ${c.priority} · lifecycle: ${c.lifecycleState} · interaction: ${c.interaction} (${c.touchCount||0})</div><div>P/D: ${c.pdZone||'none'} · aligned: ${c.pdAligned==null?'unknown':c.pdAligned?'yes':'no'} · boundary: ${c.boundarySource}</div><div>Известна с: ${new Date(c.knownAt).toLocaleString('ru-RU')}</div><div>Жизнь: ${new Date(c.originAt).toLocaleString('ru-RU')} → ${c.endAt?new Date(c.endAt).toLocaleString('ru-RU'):'сейчас'}</div><div>Armed: ${c.armedAt?new Date(c.armedAt).toLocaleString('ru-RU'):'—'} · consumed: ${c.consumedAt?new Date(c.consumedAt).toLocaleString('ru-RU'):'—'} · failed: ${c.failedAt?new Date(c.failedAt).toLocaleString('ru-RU'):'—'} · retired: ${c.retiredAt?new Date(c.retiredAt).toLocaleString('ru-RU'):'—'}</div>`;chart.timeScale().setVisibleRange({from:time(c.originAt-20*14_400_000),to:time((c.endAt||last)+20*14_400_000)})}else{$('poiZoneDetail').innerHTML='';chart.timeScale().fitContent()}}
