...
function liqCandidates(){const d=$('liqDirection').value;return(data?.liquidityPoi?.candidates||[]).filter(x=>d==='all'||x.direction===d)}
...
function renderLiquidity(){if(!data||!liqMode)return;clearOverlays();restoreMainCandles();markersPlugin.setMarkers([]);const c=currentLiq(),xs=liqCandidates();if(!c){$('liqStatus').textContent='Liquidity POI candidates: 0';return}const end=time(data.candles.at(-1).timestamp),from=time(c.originAt),cols={density20:'#ffbd5b',density30:'#a98bff',density40:'#5b8cff',atr025:'#64748b',atr050:'#64748b',atr075:'#475569',atr100:'#334155'};line([{time:from,value:c.near},{time:end,value:c.near}],{color:c.direction==='long'?C.green:C.red,lineWidth:3});for(const [k,v] of Object.entries(c.boundaries))line([{time:from,value:v},{time:end,value:v}],{color:cols[k],lineWidth:k.startsWith('density')?2:1,lineStyle:LightweightCharts.LineStyle.Dashed});markersPlugin.setMarkers(c.pivotTimes.map((at,i)=>({time:time(at),position:c.direction==='long'?'belowBar':'aboveBar',color:C.purple,shape:'circle',size:1,text:`P${i+1}`})));const old=liqReviews()[c.id];$('liqChoice').value=old?.choice||'';$('liqNote').value=old?.note||'';$('liqStatus').textContent=`${liqIndex+1}/${xs.length} · ${c.direction.toUpperCase()} · pivots ${c.pivotCount} · age ${c.ageBars}×4h · score ${c.score.toFixed(1)}`;$('liqDetail').innerHTML=`<div><b>Near wick:</b> ${fmtP(c.near)} · ATR ${fmtP(c.atr)}</div>${Object.entries(c.boundaries).map(([k,v])=>`<div>${k}: ${fmtP(v)} · depth ${(Math.abs(v-c.near)/c.atr).toFixed(2)} ATR</div>`).join('')}`;chart.timeScale().setVisibleRange({from:time(c.originAt-20*14_400_000),to:time(c.originAt+80*14_400_000)})}
function toggleLiquidity(){liqMode=!liqMode;$('liqControls').style.display=liqMode?'block':'none';$('liqToggle').textContent=liqMode?'Закрыть':'Открыть';if(liqMode){refinedMode=false;labMode=false;liqIndex=0;renderLiquidity()}else{restoreMainCandles();redraw()}}
function moveLiq(n){const xs=liqCandidates();if(!xs.length)return;liqIndex=(liqIndex+n+xs.length)%xs.length;renderLiquidity()}
function saveLiq(){const c=currentLiq();if(!c||!$('liqChoice').value)return;const x=liqReviews();x[c.id]={id:c.id,symbol:data.dataset.symbol,choice:$('liqChoice').value,note:$('liqNote').value.trim(),near:c.near,boundary:$('liqChoice').value==='wrong-zone'?null:c.boundaries[$('liqChoice').value],atr:c.atr,recordedAt:new Date().toISOString()};localStorage.setItem(LIQ_REVIEW_KEY,JSON.stringify(x));moveLiq(1)}
...
function redraw(){if(!data)return;if(liqMode){renderLiquidity();return}if(refinedMode){renderRefined();return}if(labMode){renderLab();return}clearOverlays();renderList();renderEvents();renderProtected();renderMarkers();renderSelected();renderDetail();renderCards()}
